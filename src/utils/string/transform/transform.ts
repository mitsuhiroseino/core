 ...
 *      - toFullWidthAlpha: 半角英字を全角英字へ変換
 *      - toFullWidthNum: 半角数字を全角数字へ変換
 *      - toFullWidthSign: 半角記号を全角記号へ変換
 *      - toFullWidthSpace: 半角スペースを全角スペースへ変換
 *  - toHalfWidth: 全角を半角へ変換。下記を個別に指定することも可能
 *      - toHalfWidthAlpha: 全角英字を半角英字へ変換
 *      - toHalfWidthNum: 全角数字を半角数字へ変換
...
export default function transform(
  ...
  if (target) {
    ...
    if (replacementTypes.some((type) => type in FUNCTIONS)) {
      ...
      // replacerを使う変換のみ
      return replacer.replace(target, replacementTypes);
    }
  } else {
    // 変換するものなし
    return target;
