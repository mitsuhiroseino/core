...
class Events<H = EventHandlers> extends DestructibleBase<EventsConfig<H>> implements IObservable<H> {
  ...
  constructor(config?: EventsConfig<H>) {
    super(config);
    const me = this,
      { handlers = {}, eventTransformation = {}, synchronizedEvents } = me.config;
    me._eventTransformation = eventTransformation as EventTransformationConfigs;
    me._synchronizedEvents = asArray(synchronizedEvents) as EventSynchronousConfig[];
  ...
  fire(type: keyof H | string, params: FireParams = {}, options?: FireOptions): boolean {
    ...
    if (eventTransformation) {
      // イベントの変形あり
      const { type: altType, convertParams, target = me, omitOriginal } = eventTransformation,
        altParams = convertParams ? convertParams(params) : params;
      toContinueProcessing = me._fire(altType, { type: altType, params: altParams, target, ...options });
      if (omitOriginal) {
        // 元のイベントは発火しない
    ...
    return toContinueProcessing && me._fire(type, { type: orgType, params, target: me, ...options });
  }
  ...
  protected _fire(type: keyof H | string, event: EventInfo): boolean {
    ...
    let toContinueProcessing = true;
    if (me._canFire()) {
      toContinueProcessing = registeredHandlers.reduce((result, registeredHandler) => {
  ...
  destructor(): void {
    this._deleteProperties(['_handlers', '_synchronizedEvents', '_eventTransformation']);
    super.destructor();
  }
}
